...
const VehicleTrackingMap: React.FC<VehicleTrackingMapProps> = ({
  ...
  const initializeWialon = async () => {
    ...
    try {
      ...
      wialonService.initialize(wialonConfig);
      await wialonService.login();

      setIsConnected(true);
      await loadVehicleData();
    } catch (err) {
      console.error('Wialon initialization failed:', err);
      setError('Не удалось подключиться к Wialon. Используется демо-режим.');
      setIsDemoMode(true);
      loadDemoData();
    } finally {
      setLoading(false);
  ...
  const loadVehicleData = async () => {
    try {
      const positions = await wialonService.getVehiclePositions();
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {isDemoMode ? 'Демо-режим' : 'Реальные данные Wialon'} •
