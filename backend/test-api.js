...
app.get('/api/routes/popular', (req, res) => {
  const popularRoutes = routes.filter(r => r.isPopular).map(route => ({
    ...route,
...
app.post('/api/routes/calculate-price', (req, res) => {
  ...
  const route = routes.find(r => r.fromCity === fromCity && r.toCity === toCity)
...
app.post('/api/bookings', (req, res) => {
  ...
  const vehicle = vehicles.find(v => v.type === vehicleType)
  const route = routes.find(r => r.fromCity === fromLocation && r.toCity === toLocation)
